...
let theater = {
    store: "강남점",
    titles: ['파묘', 'Dune2', '웡카', '가여눅 것들', '포켓몬스터'],
    
    showMovieList() {
        console.log(this.store);

        /* 화살표 함수의 this 는 나를 호출한 대상이 아니라 상위 레벨의 this를 가리킨다. */
        /* forEach: Array에서 제공하는 메소드로 배열의 요소별로 돌아가면 콜백 함수를 실행하는 함수 */
        this.titles.forEach(
            title => console.log(this.store + ": " + title)     // this.store : 강남점
            )                                                   // 반드시 화살표 함수를 써야 하는 경우도 있다. 
...
let test = function() {
    ...
    const arrowFunc = () => console.log(arguments); // 10과 20이 아니라 상위의 function이 가진 Arguments를 반환 [Arguments] { '0': 1, '1': 2, '2': 3, '3': 4, '4': 5 }
    arrowFunc(10,20);
}
...
/*
    ...
    위와 같은 특징들은 콜백 함수 내부의 this 가 외부 함수의 this와 다르기 때문에 발생하는 문제를 해결하기 위해 
    의도적으로 설계된 것이라 할 수 있다.(arguments도 마찬가지)
    따라서 화살표 함수를 사용할 때는 어ㄴ정도 개념을 정확히 인지하고 써야한다. 
*/
