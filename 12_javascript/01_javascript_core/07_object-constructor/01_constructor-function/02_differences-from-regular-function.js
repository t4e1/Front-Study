/* 02. differences-from regular function(일반 함수와의 차이점) */
...
function Dog(name, age) {
    ...
    /* undefined 는 falsy 하기 때문에 if문에서 !를 붙여줘서 트루를 만든다  */
    if(!new.target) {                           // new 없이 함수 호출 시 true가 되게 하는 구문
        return new Dog(name, age);              // new 연산자와 함께 생성자 함수를 재귀 호출하여 생성 된 인스턴슬르 반환한다.
    }
